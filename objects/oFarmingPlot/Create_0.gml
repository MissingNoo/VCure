...
state.add("Plant", {
    ...
    step : function() {
        ...
        if (input_check_pressed("accept") and soilwindowx[0] == soilwindowx[1]) {
            state.change("Seed");
        }
        if (input_check_pressed("cancel")) { 
            state.change("Menu");
    ...
    draw : function() {
        ...
        for (var i = 0; i < array_length(global.farmsoils); i++) {
            ...
            scribble($"[fa_middle]{_name}").scale(2.25).draw(_x + 50 + 40, _y + offset);
            scribble($"[fa_right][fa_middle]x {global.farmsoils[i].amount}").scale(2.25).draw(_x + _w - 50, _y + offset);
            offset += 60;
        }
    }
});
state.add_child("Plant", "Seed", {
    ...
    draw : function() {
        ...
        for (var i = 0; i < array_length(global.crops); i++) {
            ...
            scribble($"[fa_middle]{_name}").scale(2.25).draw(_x + 50 + 40, _y + offset);
            scribble($"[fa_right][fa_middle]x {global.crops[i].amount}").scale(2.25).draw(_x + _w - 50, _y + offset);
            offset += 60;
        }
    }
});
